...
void imprime_players(player *p1, player *p2){
	al_draw_filled_rectangle(p1->x - p1->side/2, p1->y + p1->side/2, p1->x + p1->side/2, p1->y - p1->side/2, al_map_rgb(0, 0, 255));
	al_draw_filled_rectangle(p2->x - p2->side/2, p2->y + p2->side/2, p2->x + p2->side/2, p2->y - p2->side/2, al_map_rgb(255, 0, 0));
}
